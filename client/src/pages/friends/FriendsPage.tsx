  ...
  return (
    <PageShell title={t('friends_title')} lang={lang}>
        ...
        <div className="rounded-2xl p-4 mb-6 flex items-center justify-between border"
          ...
          <div>
            <p className="font-arabic text-xs mb-1" style={{ color: 'rgba(220,210,255,0.4)' }}>
              {lang === 'ar' ? 'رقمك (شاركه مع أصدقائك)' : 'Your number (share with friends)'}
            </p>
            <p className="font-bold font-mono" style={{ color: '#F0E9FF', fontSize: 26, lineHeight: 1.1, letterSpacing: 2 }}>
              {(profile?.username || '').split('#')[1] || '...'}
            </p>
            <p className="font-arabic mt-0.5" style={{ color: 'rgba(220,210,255,0.3)', fontSize: 10, direction: 'ltr', textAlign: dir === 'rtl' ? 'right' : 'left' }}>
              {profile?.username || ''}
            </p>
          </div>
          <button
            ...
            style={{ background: copied ? 'rgba(80,200,120,0.15)' : 'rgba(167,139,250,0.12)', border: `1px solid ${copied ? 'rgba(80,200,120,0.4)' : 'rgba(167,139,250,0.3)'}`, color: copied ? '#50C878' : '#A78BFA' }}>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="friends" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}>
              {friends.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="font-arabic" style={{ color: 'rgba(220,210,255,0.35)', fontSize: 14 }}>{t('friends_empty')}</p>
                  <p className="font-arabic mt-1" style={{ color: 'rgba(220,210,255,0.2)', fontSize: 12 }}>
          ...
          {tab === 'requests' && (
            <motion.div key="requests" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}>
              {requests.length === 0 ? (
                <div className="text-center py-16">
                  <p className="text-4xl mb-3">📩</p>
                  <p className="font-arabic" style={{ color: 'rgba(220,210,255,0.35)', fontSize: 14 }}>{t('friends_no_requests')}</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {requests.map(f => (
                    <div key={f.uid} className="rounded-xl p-4 flex items-center gap-3 border"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-arabic font-bold truncate" style={{ color: '#F0E9FF', fontSize: 15 }}>{f.displayName}</p>
                        <p className="font-arabic text-xs" style={{ color: 'rgba(220,210,255,0.35)', direction: 'ltr' }}>{f.username}</p>
                      </div>
          ...
          {tab === 'add' && (
            <motion.div key="add" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}>
              <div className="rounded-2xl p-5 border" style={{ background: 'rgba(255,255,255,0.03)', borderColor: 'rgba(167,139,250,0.15)' }}>
                <p className="font-arabic mb-1 font-bold" style={{ color: '#F0E9FF' }}>{t('friends_add')}</p>
                <p className="font-arabic text-xs mb-4" style={{ color: 'rgba(220,210,255,0.35)' }}>
                  {lang === 'ar'
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(220,210,255,0.95)', direction: 'ltr', letterSpacing: 1 }}
                  />
                  <button
                    onClick={handleSendRequest}
                    disabled={loading || !searchQuery.trim()}
                    className="px-4 py-2.5 rounded-xl font-arabic font-bold text-sm transition-all disabled:opacity-40"
                    style={{ background: 'linear-gradient(135deg, #A78BFA, #5B21B6)', color: '#0A0814' }}>
        ...
        {confirmRemove && (
          ...
          >
            <motion.div
              ...
              style={{
                background: 'linear-gradient(160deg, #241810 0%, #15102F 100%)',
              ...
              <div className="text-center mb-4">
                ...
                <h2 className="font-arabic font-bold mb-1" style={{ fontSize: 18, color: '#F0E9FF' }}>
                  {lang === 'ar' ? 'تأكيد الحذف' : 'Confirm Remove'}
                </h2>
                <p className="font-arabic" style={{ fontSize: 13, color: 'rgba(220,210,255,0.65)' }}>
                  {lang === 'ar' ? 'هل أنت متأكد من حذف' : 'Are you sure you want to remove'}{' '}
                  <span style={{ color: '#F0E9FF', fontWeight: 700 }}>{confirmRemove.displayName}</span>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  style={{ background: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.12)', color: 'rgba(220,210,255,0.7)' }}>
...
function FriendCard({ friend, busy, actionLabel, actionStyle, onAction, lang }: {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-arabic font-bold truncate" style={{ color: '#F0E9FF', fontSize: 15 }}>{friend.displayName}</p>
        <div className="flex items-center gap-3 mt-0.5">
          <span className="font-arabic text-xs" style={{ color: 'rgba(220,210,255,0.3)', direction: 'ltr' }}>{friend.username}</span>
          <span className="font-arabic text-xs" style={{ color: 'rgba(167,139,250,0.5)' }}>{lang === 'ar' ? 'لv' : 'Lv'}{friend.level}</span>
          {friend.wins !== undefined && <span className="font-arabic text-xs" style={{ color: 'rgba(220,210,255,0.3)' }}>🏆 {friend.wins}</span>}
        </div>
      </div>
      {friend.inGame && (
        <button
