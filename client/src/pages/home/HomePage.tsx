  ...
  return (
    <motion.button
      ...
      style={{
        width: 36,
        height: 34,
        background: canClaim ? 'rgba(229,188,124,0.18)' : 'rgba(255,255,255,0.04)',
        border: `1px solid ${canClaim ? 'rgba(229,188,124,0.55)' : 'rgba(255,255,255,0.08)'}`,
        boxShadow: canClaim ? '0 0 14px rgba(229,188,124,0.35)' : 'none',
        cursor: 'pointer',
      }}>
      <span style={{ fontSize: 18, lineHeight: 1 }}>{canClaim ? '🎁' : '📦'}</span>
      {canClaim && (
        <span className="absolute rounded-full animate-pulse"
          style={{ top: -3, right: -3, width: 9, height: 9, background: '#E04030', border: '1.5px solid #1F1810' }} />
      )}
    </motion.button>
  );
}
...
function WheelNavButton({ onOpen, lang }: { onOpen: () => void; lang: string }) {
  ...
  return (
    <motion.button
      ...
      style={{
        width: 36, height: 34,
        background: canSpin ? 'rgba(229,188,124,0.20)' : 'rgba(255,255,255,0.04)',
        border: `1px solid ${canSpin ? 'rgba(229,188,124,0.55)' : 'rgba(255,255,255,0.08)'}`,
        boxShadow: canSpin ? '0 0 14px rgba(229,188,124,0.35)' : 'none',
        cursor: 'pointer',
      }}>
      <span style={{ fontSize: 18, lineHeight: 1 }}>{canSpin ? '🎡' : '🎰'}</span>
      {canSpin && (
        <span className="absolute rounded-full animate-pulse"
          style={{ top: -3, right: -3, width: 9, height: 9, background: '#FBF3DB', border: '1.5px solid #1F1810' }}/>
      )}
    </motion.button>
  );
}
...
function TournamentStrip({ lang, onOpen }: { lang: string; onOpen: () => void }) {
  ...
  return (
    <motion.button
      ...
      style={{
        background: active
          ? 'linear-gradient(90deg, rgba(229,188,124,0.22) 0%, rgba(168,124,58,0.14) 100%)'
          : 'linear-gradient(90deg, rgba(229,188,124,0.14) 0%, rgba(168,124,58,0.06) 100%)',
        border: `1.5px solid ${active ? 'rgba(229,188,124,0.65)' : 'rgba(229,188,124,0.40)'}`,
        boxShadow: '0 4px 18px rgba(0,0,0,0.35), 0 0 22px rgba(229,188,124,0.20)',
        cursor: 'pointer',
      }}>
      {/* Trophy icon */}
      <div className="rounded-xl flex items-center justify-center shrink-0"
        style={{
          ...
          border: '1px solid rgba(229,188,124,0.55)',
          fontSize: 26,
        }}>🏆</div>

      <div className="flex-1 min-w-0 text-start">
        <div className="font-arabic font-bold flex items-center gap-2"
          style={{ fontSize: 15, color: '#FBF3DB' }}>
        ...
        <p className="font-arabic mt-0.5"
          style={{ fontSize: 11.5, color: 'rgba(251,243,219,0.65)' }}>
          {subtitle}
        </p>
      </div>

      <span className="shrink-0" style={{ fontSize: 18, color: '#FBF3DB' }}>
        {lang === 'ar' ? '‹' : '›'}
      </span>
    </motion.button>
  );
...
function MissionsStrip({ onOpen, lang, profileXp, profileWins, profileGames, profileStreak }: {
  ...
  return (
    <motion.button
      ...
      style={{
        background: hot
          ? 'linear-gradient(90deg, rgba(229,188,124,0.18) 0%, rgba(229,188,124,0.08) 100%)'
          : 'linear-gradient(90deg, rgba(229,188,124,0.06) 0%, rgba(255,255,255,0.02) 100%)',
        border: `1px solid ${hot ? 'rgba(229,188,124,0.55)' : 'rgba(229,188,124,0.18)'}`,
        boxShadow: hot ? '0 0 16px rgba(229,188,124,0.30)' : 'none',
        cursor: 'pointer',
      }}>
      {/* Icon */}
      <div className="rounded-lg flex items-center justify-center shrink-0"
        style={{
          width: 32, height: 32,
          background: hot ? 'rgba(229,188,124,0.25)' : 'rgba(0,0,0,0.20)',
          border: '1px solid rgba(229,188,124,0.25)',
          fontSize: 18,
        }}>🎯</div>
      ...
      <div className="flex-1 min-w-0 text-start">
        <div className="font-arabic font-bold flex items-center gap-2"
          style={{ fontSize: 12.5, color: hot ? '#FBF3DB' : 'rgba(229,188,124,0.85)' }}>
        ...
        <div className="mt-1.5 flex items-center gap-2">
          ...
          <div className="flex-1" style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
            <div style={{
              ...
              height: '100%',
              background: 'linear-gradient(90deg, #A07338, #FBF3DB)',
              transition: 'width .35s ease',
            }}/>
          </div>
          <span className="font-mono shrink-0"
            style={{ fontSize: 9.5, color: 'rgba(251,243,219,0.5)' }}>
      ...
      <span className="shrink-0" style={{
        fontSize: 16, color: hot ? '#FBF3DB' : 'rgba(251,243,219,0.45)',
      }}>{lang === 'ar' ? '‹' : '›'}</span>
    </motion.button>
  );
}
  ...
  return (
    <div className="rounded-xl flex flex-col items-center justify-center"
      style={{
        ...
        border: '1px solid rgba(229,188,124,0.20)',
      }}>
      <span style={{ fontSize: 12, lineHeight: 1 }}>{icon}</span>
      <span className="font-bold" style={{ fontSize: 13, color: '#FBF3DB', lineHeight: 1.05, marginTop: 1 }}>
        {value}
      </span>
      <span className="font-arabic" style={{ fontSize: 8.5, color: 'rgba(251,243,219,0.45)', lineHeight: 1, marginTop: 1 }}>
        {label}
      </span>
    </div>
  );
...
function QuickActionsTriad({
  ...
  return (
    <div className="grid grid-cols-3 gap-2.5 mb-5">
      {tiles.map(t => {
        const accent =
          t.accent === 'gold'   ? { bg1: 'rgba(229,188,124,0.18)', bg2: 'rgba(120,80,20,0.10)', border: 'rgba(229,188,124,0.55)', text: '#FBF3DB', glow: 'rgba(229,188,124,0.25)' } :
          t.accent === 'purple' ? { bg1: 'rgba(196,149,255,0.16)', bg2: 'rgba(80,40,120,0.10)',  border: 'rgba(196,149,255,0.55)', text: '#C495FF', glow: 'rgba(196,149,255,0.20)' } :
                                  { bg1: 'rgba(251,243,219,0.10)', bg2: 'rgba(40,28,12,0.85)',   border: 'rgba(229,188,124,0.30)', text: 'rgba(251,243,219,0.85)', glow: 'rgba(0,0,0,0.30)' };
        return (
          <motion.button
            key={t.key}
            ...
            <p className="font-arabic leading-none"
              style={{ fontSize: 10, color: 'rgba(251,243,219,0.5)' }}>
              {t.sub}
            </p>
            {t.live && (
              <motion.span
      ...
      <motion.button
        ...
        style={{
          ...
          background: hot
            ...
            : 'radial-gradient(circle at 30% 30%, rgba(229,188,124,0.20), rgba(60,40,16,0.45) 70%)',
          border: `1.5px solid ${hot ? 'rgba(255,224,122,0.85)' : 'rgba(229,188,124,0.40)'}`,
          boxShadow: hot
          ...
          >
            <motion.div
              ...
              style={{
                ...
                border: '1.5px solid rgba(229,188,124,0.35)',
                boxShadow: '0 30px 80px rgba(0,0,0,0.85), 0 0 60px rgba(229,188,124,0.22)',
                maxWidth: 460,
                marginTop: 60,
                padding: 16,
              }}>
              ...
              <div className="flex flex-col gap-2">
                {rows.map(r => (
                  <motion.button
                    ...
                    style={{
                      background: r.hot
                        ? 'linear-gradient(90deg, rgba(229,188,124,0.20) 0%, rgba(40,28,12,0.85) 100%)'
                        : 'linear-gradient(90deg, rgba(255,255,255,0.04) 0%, rgba(20,16,10,0.85) 100%)',
                      border: `1.5px solid ${r.hot ? 'rgba(229,188,124,0.55)' : 'rgba(229,188,124,0.18)'}`,
                      boxShadow: r.hot ? '0 0 16px rgba(229,188,124,0.20)' : 'none',
                      cursor: 'pointer',
                    }}>
                    <div className="rounded-2xl flex items-center justify-center shrink-0"
                      style={{
                        ...
                        background: r.hot
                          ...
                          : 'rgba(0,0,0,0.30)',
                        border: `1px solid ${r.hot ? 'rgba(229,188,124,0.55)' : 'rgba(255,255,255,0.06)'}`,
                        fontSize: 24,
                      }}>
                      {r.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="font-arabic font-bold truncate"
                          style={{ fontSize: 13, color: r.hot ? '#FBF3DB' : 'rgba(229,188,124,0.85)' }}>
                          {r.title}
                        </p>
                        {r.badge && (
                          <span className="rounded-full font-bold flex items-center justify-center"
                            style={{
                              ...
                              background:
                                ...
                                r.badge.tone === 'gold'   ? '#FBF3DB' :
                                                            '#C495FF',
                              color: r.badge.tone === 'gold' ? '#100A05' : '#fff',
                            }}>
                            {r.badge.text}
                          </span>
                        )}
                      ...
                      <p className="font-arabic mt-0.5 truncate"
                        style={{ fontSize: 11, color: 'rgba(251,243,219,0.55)' }}>
                        {r.sub}
                      </p>
                    </div>
                    <span className="shrink-0" style={{
                      fontSize: 18, color: r.hot ? '#FBF3DB' : 'rgba(251,243,219,0.45)',
                    }}>{lang === 'ar' ? '‹' : '›'}</span>
                  </motion.button>
                ))}
              </div>
      ...
      {tiles.map(t => (
        <motion.button
          ...
          style={{
            ...
            background: t.hot
              ? 'linear-gradient(160deg, rgba(229,188,124,0.18) 0%, rgba(40,28,12,0.95) 100%)'
              : 'linear-gradient(160deg, rgba(255,255,255,0.05) 0%, rgba(20,16,10,0.85) 100%)',
            border: `1.5px solid ${t.hot ? 'rgba(229,188,124,0.55)' : 'rgba(229,188,124,0.18)'}`,
            boxShadow: t.hot
              ? '0 4px 16px rgba(0,0,0,0.45), 0 0 18px rgba(229,188,124,0.25)'
          ...
          <div
            ...
            style={{
              ...
              background: t.hot
                ? 'radial-gradient(circle at 30% 30%, rgba(255,224,122,0.45), rgba(168,124,58,0.20) 70%)'
                : 'rgba(0,0,0,0.30)',
              border: `1px solid ${t.hot ? 'rgba(229,188,124,0.55)' : 'rgba(255,255,255,0.06)'}`,
              fontSize: 24,
            }}>
            {t.icon}
          </div>
          <p className="font-arabic font-bold"
            style={{ fontSize: 12, color: t.hot ? '#FBF3DB' : 'rgba(251,243,219,0.78)', lineHeight: 1.1 }}>
            {t.label}
          </p>
          {t.badge && (
            <motion.span
              ...
              style={{
                ...
                background:
                  ...
                  t.badge.tone === 'gold'   ? '#FBF3DB' :
                                              '#C495FF',
                color: t.badge.tone === 'gold' ? '#100A05' : '#fff',
                border: '1.5px solid #1F1810',
              }}>
              {t.badge.text}
            </motion.span>
          )}
...
// ── Avatar ────────────────────────────────────────────────────────────────────
const AV_COLORS = ['#E5BC7C','#4A90D9','#50C878','#E74C3C','#9B59B6','#E67E22','#1ABC9C','#E91E63'];
...
function XpBar({ xp, lang, onClick }: { xp: number; lang?: string; onClick?: () => void }) {
  ...
  return (
    <div onClick={onClick}
      ...
      <div style={{ height: 6, borderRadius: 4, background: 'rgba(229,188,124,0.12)', overflow: 'hidden', width: '100%', border: '1px solid rgba(229,188,124,0.15)' }}>
        <div style={{ width: `${Math.max(2, pct)}%`, height: '100%', background: 'linear-gradient(90deg, #A07338, #FBF3DB, #FBF3DB)', borderRadius: 4, transition: 'width .5s ease', boxShadow: '0 0 8px rgba(229,188,124,0.5)' }}/>
      </div>
      <span className="font-mono"
        style={{
          fontSize: 9, color: 'rgba(251,243,219,0.5)',
          marginTop: 2, display: 'block',
        }}>
        {xpInLevel.toLocaleString()} / {xpForNextLevel.toLocaleString()} XP
        {onClick && <span style={{ marginInlineStart: 6, color: 'rgba(229,188,124,0.6)' }}>
...
const GAME_KIND_META: Record<GameType, { icon: string; ar: string; en: string; tagline: { ar: string; en: string }; accent: string; bg: string; glow: string }> = {
  check: {
    ...
    accent: '#FBF3DB',
    bg: 'linear-gradient(135deg, #1F1810 0%, #1F1810 100%)',
    glow: 'rgba(229,188,124,0.45)',
  },
  ludo: {
    icon: '🎲',
    ar: 'لودو', en: 'Ludo',
    ...
    accent: '#D9A441',
    bg: 'linear-gradient(135deg, #2A1808 0%, #1F1810 100%)',
...
function GamePicker({ gameKind, onSelect, lang }: { gameKind: GameType; onSelect: (g: GameType) => void; lang: string }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <p className="font-arabic text-center" style={{ fontSize: 11, color: 'rgba(251,243,219,0.45)', letterSpacing: 1 }}>
          ...
          return (
            ...
            >
              ...
              <div className="relative flex flex-col items-center gap-1">
                ...
                <span className="font-bold" style={{ fontSize: 16, color: isSel ? m.accent : 'rgba(251,243,219,0.85)' }}>
                  {isAr ? m.ar : m.en}
                </span>
                <span style={{ fontSize: 10, color: isSel ? `${m.accent}AA` : 'rgba(251,243,219,0.35)' }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: '1px solid rgba(229,188,124,0.28)',
        borderRadius: 999,
          ...
          <motion.button
            ...
            style={{
              ...
              background: isSel
                ...
                : 'transparent',
              color: isSel ? t.accent : 'rgba(251,243,219,0.65)',
              fontSize: 11.5,
              cursor: 'pointer',
              transition: 'color .2s',
            }}>
...
// ── Avatar stepper: − [4 / 10  👤👤👤👤·····] + ──────────────────────────────
const SEAT_COLORS = ['#E5BC7C','#4A90D9','#50C878','#E74C3C','#9B59B6','#E67E22','#1ABC9C','#E91E63','#3DB7B7','#FF6B7A'];
function PlayerStepper({ value, onChange, min, max, accent, lang, label }: {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-3">
      <span className="font-arabic" style={{ fontSize: 12, color: 'rgba(251,243,219,0.55)' }}>{label}</span>
      ...
      <div className="flex items-center gap-3 w-full" style={{ maxWidth: 280 }}>
        ...
        <div className="flex-1 flex flex-col items-center rounded-2xl py-2"
          ...
          <span className="font-arabic" style={{ fontSize: 10, color: 'rgba(251,243,219,0.4)', marginTop: 2 }}>
...
function CoinStepper({ value, onChange, accent, max, lang }: {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-3">
      <span className="font-arabic" style={{ fontSize: 12, color: 'rgba(251,243,219,0.55)' }}>
      ...
      <div className="flex items-center gap-3 w-full" style={{ maxWidth: 320 }}>
        ...
        <div className="flex-1 flex flex-col items-center rounded-2xl py-2 px-3"
          ...
          <span className="font-arabic mt-1" style={{ fontSize: 10, color: 'rgba(251,243,219,0.4)' }}>
      ...
      <div className="w-full" style={{ maxWidth: 320 }}>
        ...
        <div className="flex justify-between font-mono mt-1" style={{ fontSize: 9, color: 'rgba(251,243,219,0.35)' }}>
...
function DifficultyCards({ value, onChange, lang }: {
  ...
  const opts = [
    ...
    { v: 'medium' as const, ar: 'متوسط', en: 'Medium', color: '#FBF3DB' },
          ...
          return (
            ...
            >
              <span className="font-arabic font-bold" style={{
                fontSize: 20,
                color: sel ? o.color : 'rgba(251,243,219,0.85)',
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                color: sel ? accent : 'rgba(251,243,219,0.55)',
    ...
    <div className="flex flex-col items-stretch gap-4">
      ...
      <motion.button
        ...
        style={{
          ...
          color: isOk ? '#100A05' : 'rgba(255,255,255,0.25)',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(20,14,8,0.88)', backdropFilter: 'blur(8px)' }}>
      <motion.div
        ...
        style={{ background: 'rgba(20,16,10,0.97)', borderColor: 'rgba(229,188,124,0.2)', minWidth: 300 }}
      >
        {/* Pulsing rings */}
        <div className="relative flex items-center justify-center" style={{ width: 90, height: 90 }}>
          {[1, 1.6, 2.2].map((scale, i) => (
            <motion.div
              ...
              style={{ width: 44, height: 44, borderColor: 'rgba(229,188,124,0.5)' }}
            />
          ))}
        ...
        <div className="text-center">
          <p className="font-arabic font-bold text-lg mb-1" style={{ color: '#FBF3DB' }}>
            {lang === 'ar' ? 'جاري البحث عن لاعبين...' : 'Searching for players...'}
          </p>
          <p className="font-arabic text-sm" style={{ color: 'rgba(251,243,219,0.4)' }}>
            {lang === 'ar' ? 'سيبدأ المباراة بمجرد اكتمال اللاعبين' : 'Match starts once players are found'}
          </p>
        </div>

        {/* Timer */}
        <div className="rounded-2xl px-8 py-3 border" style={{ background: 'rgba(229,188,124,0.06)', borderColor: 'rgba(229,188,124,0.15)' }}>
          <p className="font-mono text-3xl font-bold" style={{ color: '#E5BC7C', letterSpacing: 4 }}>
            {mm}:{ss}
          </p>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(20,14,8,0.88)', backdropFilter: 'blur(8px)' }}>
      <motion.div
        ...
        animate={{ opacity: 1, scale: 1 }}
        className="flex flex-col items-center gap-6 rounded-3xl p-10 border"
        style={{ background: 'rgba(20,16,10,0.97)', borderColor: 'rgba(229,188,124,0.2)', minWidth: 280 }}
      >
        <motion.div
          animate={{ rotate: 360 }}
          transition={{ duration: 1.2, repeat: Infinity, ease: 'linear' }}
          style={{ width: 56, height: 56, borderRadius: '50%', border: '3px solid rgba(229,188,124,0.15)', borderTopColor: '#E5BC7C' }}
        />
        <div className="text-center">
          <p className="font-arabic font-bold text-lg mb-1" style={{ color: '#FBF3DB' }}>
        ...
        <GlassCard inner className="flex items-center justify-between gap-3" style={{ padding: '10px 12px' }}>
          ...
          <Link to="/" className="flex flex-col items-center justify-center gap-0">
            <span className="font-display tracking-widest"
              style={{
                fontSize: 26, color: '#FBF3DB',
                letterSpacing: '0.20em', lineHeight: 1,
                textShadow: '0 2px 12px rgba(229,188,124,0.45), 0 0 28px rgba(229,188,124,0.30)',
              }}>
              CHECK
            </span>
            {profile && (
              <span className="font-arabic mt-0.5" style={{ fontSize: 10, color: 'rgba(251,243,219,0.45)' }}>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="flex items-center gap-1 rounded-full px-2.5 py-1"
              style={{
                background: 'linear-gradient(135deg, rgba(229,188,124,0.20), rgba(229,188,124,0.10))',
                border: '1.5px solid rgba(229,188,124,0.55)',
                boxShadow: 'inset 0 -2px 6px rgba(0,0,0,0.45), 0 0 12px rgba(229,188,124,0.20)',
              }}>
              <span style={{ fontSize: 14 }}>🪙</span>
              <span className="font-bold" style={{ fontSize: 12, color: '#FBF3DB' }}>{coins.toLocaleString()}</span>
            </div>
            {profile && (
              <RewardsLauncher
                lang={lang}
              ...
              <div className="flex items-center gap-4 mb-4">
                <Link to="/profile" style={{ textDecoration: 'none' }} className="relative shrink-0">
                  <div className="rounded-full flex items-center justify-center"
                    style={{
                      width: 72, height: 72,
                      background: 'radial-gradient(circle at 30% 30%, rgba(229,188,124,0.55), rgba(40,28,12,0.55) 70%)',
                      border: '2px solid rgba(229,188,124,0.75)',
                      boxShadow: '0 0 22px rgba(229,188,124,0.45), inset 0 -3px 10px rgba(0,0,0,0.45)',
                      padding: 4,
                    }}>
                  ...
                  <div className="absolute rounded-full font-bold flex items-center justify-center"
                    style={{
                      ...
                      background: 'linear-gradient(135deg, #FBF3DB, #E5BC7C)',
                      fontSize: 12, color: '#100A05', fontWeight: 800,
                      border: '2px solid #1F1810',
                      padding: '0 6px',
                      boxShadow: '0 0 14px rgba(229,188,124,0.65)',
                    }}>
                    {level}
                  </div>
                </Link>
                <div className="flex-1 min-w-0">
                  <h2 className="font-display tracking-wider truncate"
                    style={{
                      fontSize: 22, color: '#FBF3DB',
                      letterSpacing: '0.06em', lineHeight: 1.05,
                      textShadow: '0 0 14px rgba(229,188,124,0.45)',
                    }}>{profile.displayName}</h2>
                  <p className="font-arabic truncate mt-1" style={{ fontSize: 11, color: 'rgba(251,243,219,0.55)' }}>
          ...
          <>
            ...
            <div className="text-center mb-5">
              <div className="flex items-center justify-center gap-3 mb-1">
                <div style={{ flex: 1, height: 1, background: 'linear-gradient(to right, transparent, rgba(229,188,124,0.3))' }}/>
                <span className="font-display tracking-widest" style={{ fontSize: 22, color: '#E5BC7C' }}>CHECK</span>
                <div style={{ flex: 1, height: 1, background: 'linear-gradient(to left, transparent, rgba(229,188,124,0.3))' }}/>
              </div>
              <p className="font-arabic" style={{ fontSize: 12, color: 'rgba(251,243,219,0.3)' }}>{t('home_subtitle')}</p>
            </div>
            ...
            <div className="mt-4 flex items-center justify-center gap-3 flex-wrap">
              {mode === 'private' && (
                <button onClick={() => setShowJoin(true)}
                  ...
                  style={{ color: 'rgba(251,243,219,0.45)', textDecoration: 'underline', textDecorationColor: 'rgba(251,243,219,0.2)' }}>
