    ...
    <PageShell
      ...
      right={myRank > 0 ? (
        <div className="rounded-full px-3 py-1.5"
          style={{ background: 'rgba(167,139,250,0.12)', border: '1.5px solid rgba(167,139,250,0.45)' }}>
          <span className="font-arabic text-xs font-bold" style={{ color: '#F0E9FF' }}>#{myRank}</span>
        </div>
      ) : undefined}
    >
      <div>
        ...
        {loading ? (
          <div className="space-y-2">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background: 'rgba(167,139,250,0.04)' }} />
            ))}
          </div>
          ...
          <div className="text-center py-20">
            ...
            <p className="font-arabic" style={{ color: 'rgba(220,210,255,0.3)' }}>{t('leaderboard_no_data')}</p>
          </div>
        ) : (
          <>
                  ...
                  return (
                    <motion.div key={entry.uid}
                      ...
                      <p className="font-arabic text-xs font-bold text-center max-w-20 truncate"
                        style={{ color: isMe ? '#A78BFA' : 'rgba(220,210,255,0.8)' }}>
                  ...
                  >
                    <div className="w-8 text-center shrink-0">
                      {isTop3
                        ...
                        : <span className="font-bold text-sm" style={{ color: 'rgba(220,210,255,0.3)' }}>{idx + 1}</span>}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-arabic font-bold truncate"
                        style={{ fontSize: 14, color: isMe ? '#F0E9FF' : isTop3 ? mc!.text : 'rgba(220,210,255,0.85)' }}>
                        {isMe ? `${entry.displayName} (${meLabel})` : entry.displayName}
                      </p>
                      <p className="font-arabic text-xs truncate" style={{ color: 'rgba(220,210,255,0.3)' }}>
                    ...
                    <div className="text-center shrink-0 hidden sm:block">
                      ...
                      <p className="font-arabic" style={{ fontSize: 10, color: 'rgba(220,210,255,0.3)' }}>{t('winRate')}</p>
                    </div>

                    <div className="text-center shrink-0 w-14">
                      <p className="font-bold text-xl leading-none"
                        style={{ color: isTop3 ? mc!.num : '#A78BFA' }}>{entry.wins}</p>
                      <p className="font-arabic" style={{ fontSize: 10, color: 'rgba(220,210,255,0.35)' }}>{t('leaderboard_victory')}</p>
                    </div>
                  </motion.div>
                );
              })}
    ...
    <button onClick={onClick}
      ...
      {badge !== undefined && badge > 0 && (
        <span className="ms-2 inline-block rounded-full px-2 font-mono"
          style={{ background: 'rgba(167,139,250,0.18)', color: '#F0E9FF', fontSize: 10, lineHeight: '17px' }}>
          {badge}
        </span>
      )}
    </button>
...
function EmptyFriends({ lang, onAdd }: { lang: string; onAdd: () => void }) {
  return (
    <div className="text-center py-12 rounded-2xl"
      ...
      <p className="font-arabic font-bold mb-2" style={{ fontSize: 16, color: '#F0E9FF' }}>
        {lang === 'ar' ? 'لا أصدقاء بعد' : 'No friends yet'}
      </p>
      <p className="font-arabic mb-5" style={{ fontSize: 12.5, color: 'rgba(220,210,255,0.55)', lineHeight: 1.7 }}>
        {lang === 'ar'
