            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  borderRadius: 999,
                  background: sel
                    ? `radial-gradient(circle at 50% 35%, ${isRecharge ? 'rgba(80,200,120,0.30)' : 'rgba(229,188,124,0.30)'}, transparent 75%)`
                    : 'transparent',
                  color: sel
                    ? (isRecharge ? '#7AC74F' : '#FBF3DB')
                ...
                {sel && (
                  <motion.span
                    ...
                    style={{
                      ...
                      background: isRecharge ? '#7AC74F' : '#FBF3DB',
                      boxShadow: `0 0 8px ${isRecharge ? 'rgba(80,200,120,0.85)' : 'rgba(229,188,124,0.85)'}`,
                    }}
                  />
                )}
              </motion.button>
          ...
          <>
            {loading ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="h-52 rounded-2xl animate-pulse" style={{ background: 'rgba(229,188,124,0.04)' }} />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        {items.map(item => {
          ...
          return (
            <div
              ...
              style={isActive ? { boxShadow: '0 0 20px rgba(229,188,124,0.25)', background: 'rgba(229,188,124,0.05)' } : { background: 'rgba(10,18,32,0.7)' }}
            >
                ...
                <div className="mt-2.5" onClick={e => e.stopPropagation()}>
                  {isActive ? (
                    ...
                    <button
                      ...
                      style={canAfford
                        ? { background: 'rgba(229,188,124,0.15)', border: '1px solid rgba(229,188,124,0.4)', color: '#E5BC7C' }
                        : { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(251,243,219,0.35)' }}
                    >
                      {isBusy ? '...' : (
                        <>
                          <span>🪙</span>
        ...
        <div className="mt-2" onClick={e => e.stopPropagation()}>
          {isEquipped ? (
            ...
            <button
              ...
              className="w-full py-1.5 rounded-lg text-xs font-arabic transition-all disabled:opacity-40 flex items-center justify-center gap-1"
              style={{ background: 'rgba(229,188,124,0.12)', border: '1px solid rgba(229,188,124,0.35)', color: '#E5BC7C' }}
            >
              {isBusy ? '...' : (
                <>
                  <span>🪙</span>
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl p-5 mb-6 border text-center"
        style={{
          background: 'linear-gradient(135deg, rgba(229,188,124,0.10) 0%, rgba(120,80,20,0.06) 100%)',
          borderColor: 'rgba(229,188,124,0.40)',
          boxShadow: '0 0 24px rgba(229,188,124,0.10)',
        }}>
        <div className="text-3xl mb-2">🚧</div>
        <p className="font-arabic font-bold text-base mb-1.5" style={{ color: '#FBF3DB' }}>
          {lang === 'ar' ? 'متجر الكوينز الحقيقي قريباً' : 'Real coin store coming soon'}
        </p>
        <p className="font-arabic" style={{ fontSize: 12, color: 'rgba(251,243,219,0.55)', lineHeight: 1.7 }}>
          {lang === 'ar'
          ...
          >
            ...
            <div className="text-center mb-4">
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <span className="text-2xl font-bold font-mono" style={{ color: 'rgba(229,188,124,0.7)' }}>
            ...
            <button
              ...
              style={{
                ...
                border: '1px dashed rgba(229,188,124,0.30)',
                color: 'rgba(229,188,124,0.55)',
              }}
            >
...
const CARD_BACK_STORE: Record<string, { bg1: string; bg2: string; accent: string }> = {
  card_classic: { bg1: '#080D22', bg2: '#040918', accent: '#E5BC7C' },
...
function BoardThemeMini({ id, size = 'lg' }: { id: string; size?: 'md' | 'lg' }) {
  ...
  return (
    <svg width={viewSize} height={viewSize} viewBox={`0 0 ${viewSize} ${viewSize}`} xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      ...
      {/* Rim ring highlight */}
      <circle cx={cx} cy={cx} r={r + rim} fill="none" stroke="rgba(229,188,124,0.18)" strokeWidth="0.8"/>
      {/* Felt */}
      <circle cx={cx} cy={cx} r={r} fill={`url(#sb-felt-${uid})`}/>
      {/* Felt inner sheen */}
      <circle cx={cx - r*0.12} cy={cx - r*0.18} r={r * 0.55} fill="rgba(255,255,255,0.025)"/>
      {/* Inner dashed ring */}
      <circle cx={cx} cy={cx} r={r * 0.78} fill="none" stroke="rgba(229,188,124,0.13)" strokeWidth="0.5" strokeDasharray="2 2"/>
      {/* CHECK watermark */}
      <text x={cx} y={cx + 2.5} textAnchor="middle" fill="rgba(229,188,124,0.12)" fontSize={size === 'lg' ? 7 : 5} fontFamily="Georgia, serif" fontWeight="bold" letterSpacing="2">CHECK</text>
      {/* Deck dot */}
      <circle cx={cx - 7} cy={cx} r="3.5" fill="rgba(0,0,0,0.35)" stroke="rgba(229,188,124,0.25)" strokeWidth="0.6"/>
      <circle cx={cx + 7} cy={cx} r="3.5" fill="rgba(255,255,255,0.08)" stroke="rgba(229,188,124,0.18)" strokeWidth="0.6"/>
    </svg>
  );
}
  ...
  if (item.category === 'avatarFrame') {
    ...
    return (
      <div className={`${wh} relative rounded-full flex items-center justify-center`}
        ...
        <span className="font-bold font-arabic" style={{ color: '#FBF3DB', fontSize: size === 'lg' ? 32 : 18 }}>ل</span>
